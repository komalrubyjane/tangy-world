...
const TicketCardInner = ({ event, idx, onBook }) => (
  <div className="w-full h-full bg-[#E7D5A4] p-4 sm:p-5 border-4 border-[#11100C] flex flex-col relative shadow-[16px_16px_0px_#11100C] sm:shadow-[20px_20px_0px_#11100C] transition-transform duration-300 group-hover:-translate-y-2">
    ...
    <div className="flex justify-between items-center font-mono text-[8.5px] sm:text-[9px] font-bold text-[#11100C] border-b-2 border-[#11100C] pb-1.5 mb-2.5">
      <span>TANGY CONCERT SERIES</span>
      <span>TICKET #TK-1974-00{idx + 1}</span>
    </div>

    <div className="w-full overflow-hidden relative border-2 border-[#11100C] bg-black shrink-0 h-44 sm:h-52">
    ...
    <div className="mt-3 flex flex-col flex-grow text-[#11100C]">
      ...
      <h3 className="display font-bold text-xl sm:text-2xl text-[#11100C] leading-none mb-2">{event.title.toUpperCase()}</h3>
      
      <div className="font-mono text-[8.5px] sm:text-[9px] tracking-widest text-[#11100C] border-y border-[#11100C]/30 py-1.5 mb-3">
        VENUE: {event.venue}<br />SOUND: {event.tags?.join(' / ')}
      </div>

      <button onClick={() => onBook(event)}
        className="mt-auto w-full flex items-center justify-center gap-2 font-mono text-xs font-bold uppercase tracking-widest bg-[#c2272a] text-[#ecdcaf] border-2 border-[#11100C] py-2.5 sm:py-3 transition-colors shadow-[3px_3px_0px_#11100C] active:scale-95">
...
export const UpcomingEvents = ({ onSelectBooking }) => {
  ...
  return (
    <section ref={sectionRef} id="sessions"
      ...
      <div className="mobile-sessions-wrap lg:hidden w-full px-4 pt-6 pb-16 flex flex-col gap-8 max-w-[420px] mx-auto">
        {events.map((event, idx) => (
          <div key={event.id} className="mobile-session-card w-full relative group min-h-[520px] h-auto">
            <TicketCardInner event={event} idx={idx} onBook={handleBookClick} />
          </div>
        ))}
        <div className="mobile-session-card w-full relative group min-h-[460px] h-auto">
          <EditorialCardInner />
        </div>
