    ...
    <div className="min-h-screen bg-[#315D73] text-[#ecdcaf] font-mono selection:bg-[#ecdcaf] selection:text-[#315D73] overflow-x-hidden pt-16 pb-20">
      ...
      <main className="w-full max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        ...
        <div className="mt-6 mb-8 text-left">
          <span className="font-mono text-[10px] font-bold text-[#d1a437] tracking-[0.3em] uppercase">PRIVATE SESSIONS // CATEGORY 03</span>
          <h1 className="font-poster text-4xl sm:text-6xl text-[#ecdcaf]">WEDDINGS &amp; RITUALS</h1>
          <p className="font-mono text-xs text-[#ecdcaf]/80 mt-2 max-w-xl">
            Acoustic Sufi and Carnatic fusion curated for intimate wedding gatherings and ceremonies —
            free from harsh digital noise and commercial playlists.
          </p>
        </div>
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 font-mono text-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input required type="text" placeholder="WEDDING VENUE / LOCATION *" value={venue} onChange={(e) => setVenue(e.target.value)} className="p-3 bg-[#241a12] border border-[#ecdcaf]/40 text-[#ecdcaf] focus:outline-none" />
              </div>
              ...
              <textarea rows={4} placeholder="TELL US ABOUT YOUR CEREMONY & PREFERRED MUSIC TYPE..." value={message} onChange={(e) => setMessage(e.target.value)} className="p-3 bg-[#241a12] border border-[#ecdcaf]/40 text-[#ecdcaf] focus:outline-none resize-none" />
              ...
              <button type="submit" disabled={submitting} className="py-4 bg-[#ecdcaf] text-[#191410] font-mono text-xs font-bold uppercase tracking-[0.2em] hover:bg-[#d1a437] border-2 border-[#191410] transition-colors shadow-[4px_4px_0px_#191410] disabled:opacity-50">
                {submitting ? 'SUBMITTING...' : 'REQUEST A WEDDING SESSION →'}
              </button>
            </form>
          )}
        </div>
