...
const ENQUIRY_TYPE = 'private_gathering';

export const PrivateGatheringsPage = () => {
  ...
  const [guests, setGuests] = useState('20-50');
    ...
    <div className="min-h-screen bg-[#315D73] text-[#ecdcaf] font-mono selection:bg-[#ecdcaf] selection:text-[#315D73] overflow-x-hidden pt-16 pb-20">
      ...
      <main className="w-full max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        ...
        <div className="mt-6 mb-8 text-left">
          <span className="font-mono text-[10px] font-bold text-[#d1a437] tracking-[0.3em] uppercase">PRIVATE SESSIONS // CATEGORY 01</span>
          <h1 className="font-poster text-4xl sm:text-6xl text-[#ecdcaf]">PRIVATE GATHERINGS</h1>
          <p className="font-mono text-xs text-[#ecdcaf]/80 mt-2 max-w-xl">
            Intimate acoustic soundscapes hosted inside private courtyards, living rooms, and rooftops —
            three to five artists, portable vintage sound, curated for small guest lists.
          </p>
        </div>
        ...
        <div className="bg-[#191410] border-4 border-[#ecdcaf] p-6 sm:p-10 shadow-[10px_10px_0px_#191410] text-left">
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 font-mono text-xs">
              ...
              <textarea rows={4} placeholder="DETAILS ABOUT YOUR GATHERING & PREFERRED MUSIC TYPE..." value={message} onChange={(e) => setMessage(e.target.value)} className="p-3 bg-[#241a12] border border-[#ecdcaf]/40 text-[#ecdcaf] focus:outline-none resize-none" />
              ...
              <button type="submit" disabled={submitting} className="py-4 bg-[#ecdcaf] text-[#191410] font-mono text-xs font-bold uppercase tracking-[0.2em] hover:bg-[#d1a437] border-2 border-[#191410] transition-colors shadow-[4px_4px_0px_#191410] disabled:opacity-50">
                {submitting ? 'SUBMITTING...' : 'REQUEST A PRIVATE GATHERING →'}
              </button>
            </form>
          )}
        </div>
