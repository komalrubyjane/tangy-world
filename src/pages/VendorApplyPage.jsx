...
export const VendorApplyPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#465667] text-[#E7D5A4] font-mono selection:bg-[#EFE2C0] selection:text-[#315B66] printNoise">
      <Navbar />
      ...
      <section className="py-20 max-w-4xl mx-auto px-6">
        <div className="bg-[#EFE2C0] paperTexture text-[#17120D] p-8 md:p-14 border-4 border-[#17120D] shadow-[20px_20px_0px_#17120D]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10 font-mono text-xs text-[#17120D]">
            <div className="bg-[#EFE2C0] paperTexture p-4 border border-[#17120D]">
          ...
          {/* FORM */}
          {submitted ? (
            <ApplicationReceivedNotice roleLabel="Vendor" statusRoute="/vendor/dashboard" />
          ) : (
            <RequireAuthToApply roleLabel="Vendor">
              <form onSubmit={handleSubmit} className="flex flex-col gap-4 font-mono text-xs">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input required type="text" value={form.businessName} onChange={set('businessName')} placeholder="BRAND / BUSINESS NAME *" className="p-3 bg-[#EFE2C0] border border-[#17120D] focus:outline-none" />
                  <input required type="email" value={form.email} onChange={set('email')} placeholder="EMAIL ADDRESS *" className="p-3 bg-[#EFE2C0] border border-[#17120D] focus:outline-none" />
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <input required type="tel" value={form.phone} onChange={set('phone')} placeholder="PHONE NUMBER *" className="p-3 bg-[#EFE2C0] border border-[#17120D] focus:outline-none" />
                  <input required type="text" value={form.category} onChange={set('category')} placeholder="PRODUCT / CATEGORY (CHAI, CRAFT, FOOD) *" className="p-3 bg-[#EFE2C0] border border-[#17120D] focus:outline-none" />
                </div>
                <textarea required rows={4} value={form.details} onChange={set('details')} placeholder="TELL US ABOUT YOUR SETUP AND MENU/PRODUCTS *" className="p-3 bg-[#EFE2C0] border border-[#17120D] focus:outline-none" />
                {error && <div className="p-3 bg-[#B5532A] text-white font-bold border border-[#17120D]">{error}</div>}
                <button type="submit" disabled={submitting} className="btn-ticket w-full py-4 text-center !bg-[#465667] !text-[#E7D7AC] font-bold uppercase tracking-widest text-sm disabled:opacity-50">
