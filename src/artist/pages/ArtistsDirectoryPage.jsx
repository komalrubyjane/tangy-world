        ...
        {filtered.map((artist) => (
          ...
          >
            ...
            <div className="w-full h-44 sm:h-56 bg-[#191410] relative border-b-2 sm:border-b-4 border-[#191410] overflow-hidden">
              <img 
                src={artist.avatar || `/artists/artist${(artist.id.charCodeAt(0) % 5) + 1}.jpg`} 
                alt={artist.name} 
                onError={(e) => {
                  e.target.onerror = null;
                  e.target.src = '/gallery/tangy1.jpg';
                }}
                className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-500"
              />

              {/* AVAILABILITY BADGE */}
              <div className="absolute top-2.5 right-2.5 px-2 py-0.5 sm:px-2.5 sm:py-1 bg-[#191410] text-[#ecdcaf] font-mono text-[8px] sm:text-[8.5px] font-bold uppercase border border-[#d1a437] flex items-center gap-1.5 z-10">
                <span className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${artist.status === 'available' ? 'bg-[#2e6834]' : 'bg-[#d1a437]'}`} />
            ...
            <div className="p-3.5 sm:p-5 flex flex-col gap-1.5 sm:gap-2">
              ...
              <div className="flex flex-wrap gap-1 mt-1 sm:mt-2">
                {artist.tags.map((t) => (
                  <span key={t} className="px-2 py-0.5 bg-[#191410] text-[#ecdcaf] font-mono text-[7.5px] sm:text-[8px] font-bold uppercase">
                    {t}
                  </span>
                ))}
              </div>
